...
> 🔎 **요약**
> 본 테스트는 전투 종료 후 보상 획득 UI가 정상적으로 표시되고,
> 보상 아이템이 결과 화면에 매핑되었는지 여부만을 검증한다.
> 실제 인벤토리 반영 여부, 수량 증가, 서버 데이터 검증은 테스트 범위에서 제외한다.

#### 과제 요구사항
- 전투 종료 후 보상 획득 UI가 정상적으로 표시되는가
- 보상 아이템 카드 UI가 화면에 존재하는가
...
**구현 특징:**
- 순수 템플릿 매칭 (OCR 미사용)
- 아이콘 UI 존재 여부만 확인 (수량 검증 안 함)
- 단계별 조기 종료 (Early Termination)
- 상세한 로그 및 스크린샷 저장
...
**구현 파일:**
- `tests/test_reward_ui_verification.py` - 보상 UI 검증 테스트
- `tests/test_reward_ui_verification_README.md` - 상세 문서
...
2. **MISSION COMPLETE 화면 확인 및 버튼 클릭**
   ...
   - `reward_confirm.png` 버튼 클릭 (5초 대기)

3. **보상 아이템 UI 확인**
   - `credit_icon.png` - 크레딧 아이콘
   - `activity_report_icon.png` - 활동 보고서 아이콘
   - 최소 1개 이상 발견 시 PASS

**성공 요인:**
1. **보상 UI의 고정적 특성**
   - MISSION COMPLETE 타이틀, 확인 버튼 모두 고유한 시각적 특징
   - 보상 아이템 아이콘은 크기/위치가 일정
   - 템플릿 매칭으로 95%+ 인식률 달성
...
3. **검증 방식의 단순성**
   - 보상 아이콘 "존재" 여부만 확인 (아이템 수량 불필요)
   - OCR 없이 UI 출현 감지만으로 검증 가능
...
**테스트 대상:**
본 테스트는 **보상 획득 UI의 정상 노출 여부**만을 검증 대상으로 한다.

**테스트 범위 제외:**
- 실제 인벤토리 반영 여부 (서버 데이터)
- 보상 아이템 수량 검증 (OCR)
- 보상 아이템 정확한 종류 판별
- 서버 데이터 무결성

**기술 제약:**
- **템플릿 의존**: 보상 아이템 아이콘 템플릿 사전 준비 필요
- **해상도 고정**: 2560x1440 기준 (다른 해상도는 좌표/템플릿 재설정)

**필요한 템플릿:**
- `mission_complete.png` - MISSION COMPLETE 타이틀
- `reward_confirm.png` - MISSION COMPLETE 확인 버튼
- `credit_icon.png` - 크레딧 아이콘
- `activity_report_icon.png` - 활동 보고서 아이콘
