...
> 🔎 **요약**
> 스킬 사용 여부는 게임 내부 상태 접근 없이는 화면 인식만으로 신뢰성 있는 검증이 불가능하다고 판단함.
   스킬 버튼 상태(쿨다운, 활성화) 및 실제 발동 여부를 화면에서 구분할 수 없으며, OCR 기반 스킬 이름 읽기도 30% 미만의 정확도를 기록하여
   **현재 제약 조건 하에서는 실용적 구현이 어려움**.
...
> 🔎 **요약**
> 전투 결과 화면에서 학생 이름과 데미지 수치를 OCR로 읽는 시도를 진행했으나, 한글 인식률 38%, 6자리 숫자 인식률 12%로 최종 4.3% 정확도를 기록.
   큰 숫자의 쉼표 처리 실패, 숫자-알파벳 혼동(0/O, 1/I), 게임 전용 폰트의 장식 효과 등이 복합적으로 작용하여
   **OCR 기술의 근본적 한계로 구현 범위에서 제외**.
...
**결론:**
한글 텍스트와 큰 숫자 조합 인식은 Tesseract OCR의 근본적 한계로
**요구 정확도(95%) 대비 크게 미달하여 구현 범위에서 제외** 판정
