...
### 목적
스테이지 내 발판 클릭, 캐릭터 이동, 전투 진입 기능을 검증합니다.
...
#### 3.2 캐릭터 마커 초기 위치 확인
- **검증 조건 (마스크 기반 매칭)**:
  1. `character_marker.png` 템플릿 매칭
  2. 마스크 사용 (투명 PNG 알파 채널 또는 외부 마스크 파일)
  3. 마커 중심 좌표 계산
- **Pass 기준**:
  - 캐릭터 마커 인식 성공
  - 초기 위치 (x, y) 좌표 기록
...
- **검증 조건 (마스크 기반 매칭)**:
  1. 3초 대기 (이동 애니메이션)
  2. `character_marker.png` 재탐색 (마스크 사용)
  3. 이동 후 위치 (x', y') 좌표 계산
  4. 이동 거리 계산: √((x'-x)² + (y'-y)²)
  5. 최소 이동 거리: 10픽셀
- **Pass 기준**:
...
- **Fail 기준**:
  - 이동 후 마커 미발견
  - 이동 거리 < 10픽셀 (캐릭터가 이동하지 않음)
- **참고**:
  - 초기 마커 인식 실패 시 이 단계 건너뜀
  - 마스크 기반 매칭으로 배경 변화에도 안정적 인식

#### 3.5 전투 진입 또는 Phase 종료
- **검증 조건 (발판 타입별)**:
  - **적 발판인 경우**:
    1. 다중 조건 전투 검증 (battle_ui, stage_info_ui, pause_button 중 2개 이상)
    2. 스테이지 맵 소멸 확인
  - **빈 발판인 경우**:
...
### 마스크 기반 템플릿 매칭
캐릭터 마커는 역삼각형 모양으로 **중앙이 비어있고 배경(맵 타일)이 이동 전후로 달라집니다**.
일반 템플릿 매칭은 배경 불일치로 실패하므로 **마스크 기반 매칭** 사용:

#### 방법 1: 투명 PNG 사용 (권장)
- 중앙 빈 공간과 배경을 투명하게 만든 PNG 사용
- 알파 채널이 자동으로 마스크로 변환됨
- 별도 마스크 파일 불필요

#### 방법 2: 외부 마스크 파일
- `character_marker_mask.png` 생성 도구 사용
- 흰색(255): 매칭에 사용 (테두리)
- 검은색(0): 무시 (중앙 빈 공간)

**참고 문서**: [docs/MASK_USAGE_GUIDE.md](docs/MASK_USAGE_GUIDE.md)

### 종합 판정
- **Pass**: 모든 검증 항목 통과 (3.4 캐릭터 이동 포함)
- **Fail**: 필수 항목 실패 (3.4는 선택적)
- **참고**: 3.2 및 3.4 실패 시에도 나머지 항목이 통과하면 전체 Pass 가능
...
### 전투 진입 여부 판단 (개선됨)
단일 템플릿이 아닌 **전투 화면 고유 UI 3가지**를 동시 확인:

1. **battle_ui.png** - 전투 UI 전체 또는 특징적 부분 (신뢰도 ≥ 0.8)
2. **stage_info_ui.png** - 상단 스테이지 정보 UI (신뢰도 ≥ 0.8)
3. **pause_button.png** - 우측 상단 일시정지 버튼 (신뢰도 ≥ 0.8)

**Pass**: 3가지 중 2가지 이상 인식
**Fail**: 1가지 이하만 인식

**장점**:
- 고정된 UI 요소 사용 (스킬 버튼처럼 캐릭터별 다르지 않음)
- OCR 불필요 (순수 템플릿 매칭)
- 오인식 확률 감소
- 1개 템플릿 실패해도 나머지로 보완

**템플릿 준비**: [TEMPLATE_GUIDE.md](TEMPLATE_GUIDE.md) 참고
...
| 1.1 | 2026-01-08 | 다중 조건 검증 전략 개선 (전투 진입: 3가지 UI 동시 확인) |
| 1.2 | 2026-01-09 | TC-003 캐릭터 마커 위치 추적 기능 추가 (마스크 기반 템플릿 매칭) |
