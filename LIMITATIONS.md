...
- **이미지 전처리**:
  - 그레이스케일 변환
  - 임계값 처리 (THRESH_BINARY)
  - 노이즈 제거 (medianBlur)
  - 3배 스케일 업 (인식률 향상)

- **빈도 기반 필터링**:
  - 5회 재시도 후 가장 많이 나온 값 채택
  - 유효 범위(0-10) 검증

#### 실패 사례
- 스킬 이펙트로 코스트 UI가 가려진 경우
- 네트워크 지연으로 UI 업데이트 중 읽기
- 폰트 변경 또는 특수 효과 적용 시

#### 보완 가능한 대안
- **개선된 OCR**:
  - 딥러닝 기반 OCR (EasyOCR, PaddleOCR)
  - 게임 폰트 학습 데이터셋 구축
  - 커스텀 OCR 모델 훈련

- **게임 지원 시**:
  - 접근성 API로 텍스트 직접 제공
  - 테스트 모드에서 OCR 친화적 폰트 사용
  - UI 값을 로그 파일로 출력
